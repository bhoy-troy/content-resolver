...
class Query:
    def __init__(self, data: dict[str, Any], configs: dict[str, Any], settings: dict[str, Any]) -> None:
        self.data = data
    ...
    def size(self, num: float, suffix: str = "B") -> str:
    ...
    @cache
    def workloads_id(self, id: str, list_all: bool = False, output_change: str = None) -> bool | list[str]:
    ...
    @cache
    def envs_id(self, id: str, list_all: bool = False, output_change: str = None) -> bool | list[str]:
        # Accepts both env and workload ID, and returns workloads that match that
        id_components = id.split(":")
    ...
    def workload_pkgs(
        ...
        if output_change:
            ...
            for repo_id in repo_ids:
                for arch in arches:
                    for _pkg_id, pkg in pkgs[repo_id][arch].items():
    ...
    @cache
    def workload_pkgs_id(self, id: str, output_change: str = None) -> list[dict[str, Any]] | list[str]:
    ...
    @cache
    def env_pkgs(self, env_conf_id: str, repo_id: str, arch: str) -> list[dict[str, Any]]:
        # Warning: mixing repos and arches works, but might cause mess on the output
    ...
    @cache
    def env_pkgs_id(self, id: str) -> list[dict[str, Any]]:
    ...
    @cache
    def workload_size(self, workload_conf_id: str, env_conf_id: str, repo_id: str, arch: str) -> int:
        # A total size of a workload (or multiple combined!)
        pkgs = self.workload_pkgs(workload_conf_id, env_conf_id, repo_id, arch)
        # TODO: change to sum([pkg["installsize"] for pkg in pkgs])
        size = 0
        for pkg in pkgs:
    ...
    @cache
    def env_size(self, env_conf_id: str, repo_id: str, arch: str) -> int:
    ...
    @cache
    def workload_size_id(self, id: str) -> int:
        # Accepts both env and workload ID, and returns pkgs for envs that match
        id_components = id.split(":")
    ...
    @cache
    def env_size_id(self, id: str) -> int:
        # Accepts both env and workload ID, and returns pkgs for envs that match
        id_components = id.split(":")
    ...
    def workload_url_slug(self, workload_conf_id: str, env_conf_id: str, repo_id: str, arch: str) -> str:
        return f"{workload_conf_id}--{env_conf_id}--{repo_id}--{arch}"

    def env_url_slug(self, env_conf_id: str, repo_id: str, arch: str) -> str:
        return f"{env_conf_id}--{repo_id}--{arch}"

    def workload_id_string(self, workload_conf_id: str, env_conf_id: str, repo_id: str, arch: str) -> str:
        return f"{workload_conf_id}:{env_conf_id}:{repo_id}:{arch}"

    def env_id_string(self, env_conf_id: str, repo_id: str, arch: str) -> str:
        return f"{env_conf_id}:{repo_id}:{arch}"

    def url_slug_id(self, any_id: str) -> str:
        return any_id.replace(":", "--")

    @cache
    def workloads_in_view(self, view_conf_id: str, arch: str, maintainer: str = None) -> list[str]:
    ...
    @cache
    def arches_in_view(self, view_conf_id: str, maintainer: str = None) -> list[str]:
    ...
    def view_buildroot_pkgs(
        ...
        for _buildroot_pkg_relations_conf_id, buildroot_pkg_relations_conf in self.configs[
        ...
        if output_change == "source_names":
            ...
            for _pkg_name, pkg in pkgs.items():
                if pkg["srpm_name"]:
                    srpms.add(pkg["srpm_name"])

            srpm_names_sorted = sorted(srpms)
            return srpm_names_sorted

        return pkgs

    @cache
    def workload_succeeded(self, workload_conf_id: str, env_conf_id: str, repo_id: str, arch: str) -> bool:
        workload_ids = self.workloads(workload_conf_id, env_conf_id, repo_id, arch, list_all=True)
    ...
    @cache
    def workload_warnings(self, workload_conf_id: str, env_conf_id: str, repo_id: str, arch: str) -> bool:
        workload_ids = self.workloads(workload_conf_id, env_conf_id, repo_id, arch, list_all=True)
    ...
    @cache
    def env_succeeded(self, env_conf_id: str, repo_id: str, arch: str) -> bool:
        env_ids = self.envs(env_conf_id, repo_id, arch, list_all=True)
    ...
    @cache
    def view_succeeded(self, view_conf_id: str, arch: str, maintainer: str = None) -> bool:
        workload_ids = self.workloads_in_view(view_conf_id, arch)
    ...
    def _srpm_name_to_rpm_names(self, srpm_name: str, repo_id: str) -> set[str]:
        all_pkgs_by_arch = self.data["pkgs"][repo_id]

        pkg_names = set()

        for _arch, pkgs in all_pkgs_by_arch.items():
            for _pkg_id, pkg in pkgs.items():
    ...
    @cache
    def view_placeholder_srpms(self, view_conf_id: str, arch: str) -> dict[str, dict[str, set[str]]]:
        if not arch:
            raise ValueError("arch must be specified, can't be None")

        workload_ids = self.workloads_in_view(view_conf_id, arch)

        placeholder_srpms: dict[str, dict[str, set[str]]] = {}
        # {
        #    "SRPM_NAME": {
        #        "build_requires": set()
        #    }
        ...
        for workload_id in workload_ids:
            ...
            for _, pkg_placeholder in workload_conf["package_placeholders"]["srpms"].items():
    ...
    @cache
    def view_maintainers(self, view_conf_id: str, arch: str) -> set[str]:
        workload_ids = self.workloads_in_view(view_conf_id, arch)
    ...
    @cache
    def maintainers(self) -> dict[str, dict[str, Any]]:
    ...
    @cache
    def view_pkg_name_details(self, pkg_name: str, view_conf_id: str) -> Any:
        raise NotImplementedError

    @cache
    def view_srpm_name_details(self, srpm_name: str, view_conf_id: str) -> Any:
        raise NotImplementedError
